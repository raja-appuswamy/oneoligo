...
void get_consensus(vector<string> &input_dataset, vector<int> &label,
                   int max_string_len, vector<string> &output_dataset,
  ...
  for (auto &c : clusters) {
    ...
    for (int digit = 0; digit < max_string_len; digit++) {
      ...
      if (true_ch != 'A' && true_ch != 'C' && true_ch != 'G' &&
          true_ch != 'T' && true_ch != 'N') {
...
void get_indexes(vector<tuple<int, int>> &similarity_results,
                 unordered_map<int, vector<int>> &indexes, int max_index_str) {

  for (int i = 0; i < max_index_str; i++) {
    indexes[i].emplace_back(i);
  }
  for (int i = 0; i < similarity_results.size(); i++) {
    indexes[get<0>(similarity_results[i])].emplace_back(
        get<1>(similarity_results[i]));
  }
  BOOST_LOG_TRIVIAL(debug) << "Size indexes: " << indexes.size();
}

vector<int> DBSCAN(unordered_map<int, vector<int>> &indexes, int nPts,
                   size_t size_dataset) {
  ...
  for (int str = 0; str < size_dataset; str++) {
    ...
    set<int> seed_set(indexes.at(str).begin() + 1,
                      indexes.at(str).end()); // +1 to exclude the point "str"
...
void oneCluster(vector<string> &input_data, size_t batch_size, int device,
                uint32_t new_samplingrange, uint32_t new_countfilter,
                Time &timer, int nPts, string dataset_name) {
  ...
  size_t num_iterations = ceil(static_cast<float>(input_data.size()) /
                               static_cast<float>(clustering_chunk_size));
  size_t chunk_num = 0;
  bool end = false;
  ...
  while (!end) {
    ...
    if (num_iterations == 1) {
      input_chunk.insert(input_chunk.end(), input_data.begin(),
                         input_data.end());
      input_data.erase(input_data.begin(), input_data.end());
      end = true;
    } else if (chunk_num == num_iterations) {
      input_chunk.insert(input_chunk.end(), total_output_dataset.begin(),
                         total_output_dataset.end());
      end = true;
    } else {
      input_chunk.insert(input_chunk.end(), input_data.begin(),
                         input_data.begin() + range);
    ...
    OutputValues ov;
    similarity_results =
        onejoin(input_chunk, batch_size, device, new_samplingrange,
                new_countfilter, timer, ov, alg::cluster, 0, dataset_name);
    timer.end_time(cluster::onejoin);

    BOOST_LOG_TRIVIAL(debug) << "\tSize of db: " << input_data.size();
    BOOST_LOG_TRIVIAL(debug)
        << "\tSize of results: " << similarity_results.size();
    ...
    tbb::parallel_sort(
        similarity_results.begin(), similarity_results.end(),
        [](idpair &e1, idpair &e2) { return get<0>(e1) < get<0>(e2); });
    ...
    BOOST_LOG_TRIVIAL(debug)
        << "Time oneDBSCAN: " << (float)timer.get_step_time(cluster::dbscan);
    ...
    BOOST_LOG_TRIVIAL(debug)
        << "Time consensus: " << (float)timer.get_step_time(cluster::consensus);
    ...
    if (end) {
      ...
      total_output_dataset.insert(total_output_dataset.end(),
                                  make_move_iterator(output_dataset.begin()),
                                  make_move_iterator(output_dataset.end()));
    }
