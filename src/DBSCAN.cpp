...
vector<int> DBSCAN(unordered_map<int, vector<int>> &indexes, int nPts,
  ...
  cout << "Number of cluster: " << cluster << " min_points: " << min_points
       << std::endl;
  return label;
}
  ...
  while (!end) {
    ...
    if (input_data.size() == 0 && total_output_dataset.size() > 0) {
      ...
      input_chunk.insert(input_chunk.end(),
                         make_move_iterator(input_data.begin()),
                         make_move_iterator(input_data.begin() + range));
    ...
    similarity_results =
        onejoin(input_chunk, batch_size, device, new_samplingrange,
                new_countfilter, timer, ov, 0, dataset_name);
    ...
    cout << "Time creating indexes sub-step: "
         << (float)std::chrono::duration_cast<std::chrono::milliseconds>(
                end_2 - start_2)
                    .count() /
                1000
    ...
    cout << "Time oneDBSCAN: " << (float)timer.get_step_time(cluster::dbscan)
         << std::endl;
    ...
    cout << "Time consensus: " << (float)timer.get_step_time(cluster::consensus)
         << std::endl;
