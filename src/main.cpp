...
int main(int argc, char **argv) {
  ...
  description.add_options()("help,h", po::bool_switch(&help), "Display help")(
      "alg,a", po::value<int>(),
      "Algorithm to use: 1-join [default], 2-cluster")(
      ...
      "verbose,v", "[optional] Print debug information")(
      "dataset_name,n", po::value<string>(),
      "[optional] Name of dataset to use in the report name")(
      "num_thread_ed_dist,t", po::value<int>(),
      "[optional] Number of thread to use for edit distance. Default 0 "
      "(hardware cuncurrency)")(
      "min_pts,p", po::value<int>(),
      "[optional] Min number of neighbours a point has to have. Default 10");
  ...
  int alg = alg::join;
  if (vm.count("alg") && (vm["alg"].as<int>() == alg::join ||
                          vm["alg"].as<int>() == alg::cluster)) {
    alg = vm["alg"].as<int>();
  }
  ...
  if (alg == alg::join) {
    ...
    oneCluster(input_data, batch_size, device, samplingrange, countfilter,
               timer, min_pts, "GEN320");
  }
