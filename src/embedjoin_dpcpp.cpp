...
void read_dataset(vector<string> &input_data, string filename) {
  ...
  BOOST_LOG_TRIVIAL(info) << "Reading dataset...";
...
void inititalize_dictory(uint8_t *dictory) {
  ...
  if (NUM_CHAR == 4) {
    ...
    dictory[static_cast<uint8_t>(' ')] = j;
	  ...
	  BOOST_LOG_TRIVIAL(error) << "input error: check the dictory of your input\n";
	  exit(-1);
  }
}
...
void parallel_embedding(
    ...
    BOOST_LOG_TRIVIAL(info) << "\t\tTask: Embedding Data\tDevice: "
  ...
  device_queue.submit([&](handler &cgh) {
    ...
    BOOST_LOG_TRIVIAL(debug) << "\t\t\tRange: (" << batch_size << ", " << NUM_STR << ", "
              << NUM_REP << ")"<<std::endl;
...
void generate_candidates(queue &device_queue,
	...
	BOOST_LOG_TRIVIAL(info) << "\t\tTask: Candidate Pairs Generation\tDevice: "
            << device_queue.get_device().get_info<info::device::name>();
  ...
  device_queue.submit([&](handler &cgh) {
    ...
    BOOST_LOG_TRIVIAL(debug) << "\t\t\tCandidate size: " << candidate_size;
...
void generate_candidates_wrapper(vector<queue> &queues,
...

  {
    ...
    vector<long> times;
    BOOST_LOG_TRIVIAL(info) << "Size (num candidates) for profiling: " << size_for_test
         << std::endl;
    ...
    while (dev < queues.size()) {
      ...
      while (iter < size_cand[dev].size() && size_cand[dev][iter] > 0) {
        ...
        BOOST_LOG_TRIVIAL(debug) << "\tSize cand[dev]: " << size_cand[dev][iter] << std::endl;
        ...
        BOOST_LOG_TRIVIAL(debug) << "\tIter " << dev << ". Start buckets at " << offset_cand
             << ": " << start_b;
        BOOST_LOG_TRIVIAL(debug) << "\tIter " << dev << ". End buckets at "
             << offset_cand + size_cand[dev][iter] - 1 << ": " << end_b
             << std::endl;
        BOOST_LOG_TRIVIAL(debug) << "\tBuckets size: " << size_buckets << std::endl;
        BOOST_LOG_TRIVIAL(debug) << "\tBuckets offset: " << buckets_offset.back();
      ...
      if (dev == queues.size() && is_profiling) {
        ...
        BOOST_LOG_TRIVIAL(info) << "\tRemaining size: " << remaining_size << std::endl;
...
void initialize_candidate_pairs(vector<queue> &queues,
  ...
  std::cout << "\tTime cand-init: count element: "
   ...
   * */
  BOOST_LOG_TRIVIAL(debug) << "\t\tSize before remove: " << buckets_delimiter.size();
  auto new_end =
  ...
  BOOST_LOG_TRIVIAL(debug) << "\t\tSize after remove: " << buckets_delimiter.size()
            << std::endl;

  timer.end_time(cand_init::filter_buck_delim);
  std::cout << "\tTime cand-init: remove element: "
  ...
  BOOST_LOG_TRIVIAL(debug) << "\tSize to allocate: " << size << std::endl;
  try {
    ...
    cout << c << " == " << size << std::endl;
	  ...
	  BOOST_LOG_TRIVIAL(error) << "Too many candidates. Reduce the number of input strings\n"
  ...
  std::vector<long> times;
  {
    ...
    while (dev < queues.size()) {
      ...
      while (iter < size_per_dev[dev].back()) {
        ...
        BOOST_LOG_TRIVIAL(debug) << "\tOffset: " << idx_oristrings[batch_hdrs[n].offset];
        size_t next_offset =
      ...
      if (dev == queues.size() && is_profiling) {
        ...
        allocate_work(times, num_dev, n_batches - number_of_testing_batches,
                      size_per_dev);
        BOOST_LOG_TRIVIAL(info) << "\tStart computing on devices..." << std::endl;
...
void verify_pairs(vector<string> &input_data, vector<size_t> &len_oristrings,
  ...
  BOOST_LOG_TRIVIAL(debug) << "Number of threads for edit distance: " << num_threads << std::endl;
...
vector<idpair> onejoin(vector<string> &input_data, size_t max_batch_size,
  ...
  if ((num_strings % max_batch_size) != 0) {
	  BOOST_LOG_TRIVIAL(debug) << "Size last batch: " << num_strings % max_batch_size;
  ...
  try {
    oristrings.resize(tot_input_size);
	  ...
	  BOOST_LOG_TRIVIAL(error) << "It is not possible allocate the requested size.";
    exit(-1);
  }
  ...
  BOOST_LOG_TRIVIAL(debug) << "Number of devices: " << queues.size() << std::endl;
  ...
  BOOST_LOG_TRIVIAL(info) << "Clear buckets" << std::endl;
  ...
  BOOST_LOG_TRIVIAL(info) << "Starting candidate processing analysis..." << std::endl;
  BOOST_LOG_TRIVIAL(info) << "\t\tCandidates size: " << candidates.size() << std::endl;
  ...
  BOOST_LOG_TRIVIAL(info) << "End candidates processing" << std::endl;
